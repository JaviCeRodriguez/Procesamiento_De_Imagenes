# 4) Modifique el tamaño del kernel, aplique cada filtro a una imagen y vea que sucede al
# modificar la dimensión del filtro. Pruebe con kernels impares.
...
def ploteo(original, result1, result2):
    fig, axes = plt.subplots(ncols=3, nrows=1, figsize=(10, 2.5))
    ax = axes.ravel()
    ax[0] = plt.subplot(131) # Imagen original
    ax[1] = plt.subplot(132) # Imagen LF
    ax[2] = plt.subplot(133) # Imagen HF

    ax[0].imshow(original, 'gray')
    ax[0].set_title('Original')

    ax[1].imshow(result1, 'gray')
    ax[1].set_title('Filtro pasa bajos')

    ax[2].imshow(result2, 'gray')
    ax[2].set_title('Filtro pasa altos')
...
try:
    ...
    img_lf_filter = low_pass_filtering(image=image, radius=30)
    img_hf_filter = high_pass_filtering(image=image, radius=30, n=5)
    ploteo(original=image, result1=img_lf_filter, result2=img_hf_filter)
    plt.show()
